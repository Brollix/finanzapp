...
async function testDiscounts() {
    ...
    try {
        ...
        const itemsWithDiscounts = receiptData.items.filter(item => item.discount && item.discount > 0);

        if (itemsWithDiscounts.length > 0) {
            itemsWithDiscounts.forEach((item, index) => {
                console.log(`\n${index + 1}. ${item.product}${item.brand ? ` (${item.brand})` : ''}`);
                console.log(`   Quantity: ${item.quantity}${item.is_weight ? 'kg' : ''}`);
            ...
            const totalItemDiscounts = itemsWithDiscounts.reduce((sum, item) => sum + ((item.discount ?? 0) as number), 0);
        ...
        if (receiptData.discounts && receiptData.discounts.length > 0) {
            ...
            const totalUnassignedDiscounts = receiptData.discounts.reduce((sum, d) => sum + d.amount, 0);
        ...
        const itemDiscountTotal = receiptData.items.reduce((sum, item) => sum + (item.discount || 0), 0);
        const unassignedDiscountTotal = receiptData.discounts?.reduce((sum, d) => sum + d.amount, 0) || 0;
        ...
        console.log(`Unassigned discounts:    $${unassignedDiscountTotal.toFixed(2)}`);
        console.log(`─────────────────────────────────────`);
