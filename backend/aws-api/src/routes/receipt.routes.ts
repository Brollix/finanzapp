...
import { saveReceipt, getReceiptById, getReceiptsByUserId } from '../services/database.service.js';
...
router.post('/process', upload.single('image'), async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.file) {
